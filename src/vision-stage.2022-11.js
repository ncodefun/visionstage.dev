...
export class Component extends HTMLElement {
	...
	#init(){
		...
		//log('pink', 'normal properties', this.id)
		let properties
		...
		for( let [prop, desc] of flat_properties) {
			...
			else if( desc.storable && !this.id /* && debug.store */){
			...
			let store_id = this.id//||'app'//||this.localName
			//log('err', 'store id:', store_id)
			let stored_val = !!store_id ? storedValue( store_id, prop) : undefined
			// log('purple', 'storedValue( store_id, prop):', storedValue( store_id, prop))
			let use_value = stored_val !== undefined ? stored_val : desc.value
			if( stored_val !== undefined){
				// log('pink', 'Got stored val:', /* store_id, */ prop,  stored_val)
				if( desc.storable)
				...
				//log('pink', 'storing initial value:', store_id, prop, desc.value)
...
export class VisionStage extends Component {
	...
	constructor(){
		...
		// fold all <button-select> components
	...
	onHashChanged( e){
		//log('check', 'onHashChanged:', location.hash)
	...
	setPageFromHash(){ // sets this.page name (coresp to [page] attribute)
		...
		//this.params.length && log('purple','Params:', this.params)
		let page_name = ''
		//log('check', 'find page in pages:', page, this.pages)
		outer:
		...
		//setTimeout( e => this.updateScrollbarClass(), 500)
	}

	// gets page object {title,path} for the current lang
	...
	_onFirstRendered(){
		...
		// else // get from section attributes page & titles
		// 	this.buildPagesData()
		this.setPageFromHash()
	}
	...
	stringOrLocale( o){
		return typeof o === 'string' ? o : o[ this.lang] || o[ this.languages[0]];
	}
...
/**
 * Helper object setup and returned by this.prop()
