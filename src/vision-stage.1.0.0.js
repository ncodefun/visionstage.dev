...
export class VisionStage extends Component {
	...
	#setPageFromHash(){ // sets this.page name (coresp to [page] attribute)
		...
		if (this.params?.[0][0].startsWith('#')){ // anchor
			const sel = CSS.escape(location.hash.slice(1))
			// log('red', 'selector:', sel) //
			setTimeout( e => {
				let elem = q('#'+sel)
				// elem && log('red', 'elem top:', elem.offsetTop)
				if (elem){
					let header_height = q('#app-header')?.offsetHeight || 0
					q('#app-content').scrollTop = elem.offsetTop - header_height
				}
			}, 200)
		}
...
VisionStage._properties = {
	...
	page: {
		...
		watcher( val, prev){
			...
			if (!this.params?.[0]?.[0].startsWith('#'))
				this.q('#app-content').scrollTop = 0
